...
public partial class RecentTransactionsComponent
{
        ...
        if (_columnsDefinition == null)
        {
            ...
            else
            {
                ...
                new ColumnDefinition<TransactionModel>[]{
                    ...
                    new () { DataField = "CreditAmount", Caption="IN", HeaderAlignment = Alignment.Right, BodyAlignment = Alignment.Right, ShowValueFunc = ReturnCreditAmountValue },
                    new () { DataField = "DebitAmount", Caption="OUT", HeaderAlignment = Alignment.Right, BodyAlignment = Alignment.Right, ShowValueFunc = ReturnDebitAmountValue },
                    new () { DataField = "Balance", Caption="BALANCE", HeaderAlignment = Alignment.Right, BodyAlignment = Alignment.Right, DataType = DataType.Currency, Format = "N2" }
                    });
    ...
    private async Task GetTransactions(bool isSearching = false)
    {
        ...
        {
            if (_pageIndex == 1)
                _recentTransactionModel = FakeData.RecentTransaction_Page1;

            if (_pageIndex == 2)
                _recentTransactionModel = FakeData.RecentTransaction_Page2;

            if (_pageIndex == 3)
                _recentTransactionModel = FakeData.RecentTransaction_Page3;
        }
    }
