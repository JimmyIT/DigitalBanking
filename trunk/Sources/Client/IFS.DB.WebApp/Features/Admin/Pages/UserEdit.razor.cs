    ...
    private async Task ShowConfirmLockUserModal()
    {
        ...
        else if (_userCreationRequestModel.ActionStatus.GetValueOrDefault().Equals(UserActionStatusEnum.AwaitingSignoff))
        {
            ...
            if (result.Confirmed)
            {
                ...
                _isJustConfirmLocked = true;
                StateHasChanged();
            }
        }
    }

    private async Task ShowModalConfirmToMoveToArchive()
    {
        ModalParameters parameters = new ModalParameters()
                .Add(nameof(CommonModalComponent.HeaderLabel), "This profile was moved to archive")
                .Add(nameof(CommonModalComponent.UseButtonOk), true);
        ...
        var modalResult = ModalSvc.Show<CommonModalComponent>($"This profile was moved to archive", parameters, options);
